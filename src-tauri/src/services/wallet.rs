...
impl WalletService {
    ...
    pub async fn request_eth_faucet(&self) -> Result<String> {
        let address = self
            .cached_address
            .as_deref()
            .ok_or_else(|| ArchivistError::WalletError("No wallet address available".into()))?;
        ...
        let client = reqwest::Client::new();
        let url = "https://faucet-arb.testnet.archivist.storage";
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|e| ArchivistError::WalletError(format!("Faucet request failed: {}", e)))?;

        let status = response.status();
        let body = response.text().await.unwrap_or_default();

        if status.is_success() {
            log::info!("ETH faucet request successful for {}", address);
            Ok(body)
        } else {
            Err(ArchivistError::WalletError(format!(
                "ETH faucet returned HTTP {}: {}",
                status, body
            )))
        }
    }

    /// Request testnet TST tokens from faucet
    pub async fn request_tst_faucet(&self) -> Result<String> {
        let address = self
            .cached_address
            .as_deref()
            .ok_or_else(|| ArchivistError::WalletError("No wallet address available".into()))?;
        ...
        let client = reqwest::Client::new();
        let url = "https://faucet-tst.testnet.archivist.storage";
        ...
        if status.is_success() {
            log::info!("TST faucet request successful for {}", address);
            Ok(body)
        } else {
            Err(ArchivistError::WalletError(format!(
                "TST faucet returned HTTP {}: {}",
                status, body
            )))
        }
    }
